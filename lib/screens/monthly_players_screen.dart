...
class MonthlyPlayersScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MonthlyPlayersScreen> createState() =>
      _MonthlyPlayersScreenState();
}

class _MonthlyPlayersScreenState extends ConsumerState<MonthlyPlayersScreen> {
  ...
  Future<void> _loadMonthlyPlayers() async {
    try {
      setState(() {
        _isLoading = true;
        _error = null;
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(selectedGame != null
            ? '4️⃣ Mensalistas - ${selectedGame.organizationName}'
            : '4️⃣ Consulta Mensalistas'),
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
        centerTitle: true,
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: _loadMonthlyPlayers,
          ),
        ],
      ),
      body: _isLoading
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text(
                'Não há jogadores mensalistas cadastrados.',
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 16),
              ElevatedButton.icon(
        ...
        child: Column(
          ...
          children: [
            ...
            if (player['preferred_foot'] != null)
              _buildInfoRow('🦵 Perna Preferida', player['preferred_foot']),
          ],
        ),
      ),
    );
